...
router.post('/create', apiAuth, async (req, res) => {
    try {
        ...
        if (req.query.password) {
            const searchedPassword = await Quiz.findOne({ password })
            if (searchedPassword) {
                return res.status(409).json({ error: 'QUIZ_PASSWORD_ALREADY_USED' })
            }
        }
...
router.post('/data', apiAuth, async (req, res) => {
    try {
        ...
        if (searchedQuiz.endTime && dateNow > searchedQuiz.endTime) {
            return res.status(200).json({ warning: 'QUIZ_ENDED', endTime: searchedQuiz.endTime })
        }
...
router.get('/search', async (req, res) => {
    try {
        if (!(req.query.text) && !(req.query.code) && !(req.query.password)) {
            return res.status(406).json({ error: 'ID_OR_FILTER_NOT_FOUND' })
        }
